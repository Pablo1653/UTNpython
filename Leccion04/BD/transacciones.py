...
#no vamos a utilizar with ya que guarda automaticamente en la BD y si hay un error hace un rollback automaticamente , por eso lo hacemos de forma manual
    conexion.autocommit=False # para que no se guarde de manera automatica 
    cursor =conexion.cursor()
    sentencia='INSERT INTO persona (nombre,apellido,email) VALUES (%s,%s,%s)'
    valores=('Maria','Delagracias','mariadelas@hotmail.com')
    cursor.execute(sentencia,valores)
    conexion.commit()#acá hacemos el commit manualmente
